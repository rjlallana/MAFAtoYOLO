...
def make_train_data():
    '''
    MAFA-Label-Train/readme-train.txt
    ...
    train = scipy.io.loadmat('MAFA-Label-Train/LabelTrainAll.mat')
    train_path = 'MAFA-Label-Train/LabelTrainAll.mat'
    # print(train.keys())
...
def mafa_to_yolo_labels(df, mode):
    path = './'+mode+'-images/labels/'
    os.mkdir(path)
    for index, row in df.iterrows():
        f = open(path+row.image_name[:-4]+'.txt','a') # a: Opens a file for appending new information to it. The pointer is placed at the end of the file. A new file is created if one with the same name doesn't exist.
        img = cv2.imread('./'+mode+'-images/images/'+row.image_name)
        x, y, w, h = bb_to_yolo(img, row.x, row.y, row.w, row.h)
